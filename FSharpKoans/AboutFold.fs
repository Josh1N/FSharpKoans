...
module ``17: Welcome to the functional fold`` =
    ...
    let ``01 A fold which sums a list`` () =
        let fold initialState xs =
            let rec folder xs initialState =
                match xs with 
                |[] -> initialState
                |a:: rest -> folder rest initialState+a
            folder xs initialState// write a function to do what's described above
    ...
    let ``02 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec folder xs initialState =
                ...
                |[] -> initialState
                |a:: rest -> folder rest initialState*a
            folder xs initialState // write a function to multiply the elements of a list
        fold 1 [99] |> should equal 99
        fold 2 [1;11] |> should equal 22
    ...
    let ``03 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec folder f state list =
                match list with
                |[] -> state
                |a::rest -> 
                    match f state a with 
                    |newState -> folder f newState rest
            folder f initialState xs // write a function to do a fold.
        fold (+) 0 [1;2;3;4] |> should equal 10
