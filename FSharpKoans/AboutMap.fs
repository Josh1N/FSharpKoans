...
module ``15: Applying a map to a list`` =
    ...
    let ``01 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec mapping xs newList =
                match xs with
                |[] -> newList
                |a::rest -> mapping (rest) (newList@[a+1])
            mapping xs [] // write a function which adds 1 to each element
    ...
    let ``02 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec mapping xs newList =
                match xs with
                |[] -> newList
                |a::rest -> mapping (rest) (newList@[a*2])
            mapping xs [] // write a function which doubles each element
   ...
   *)
    ...
    let ``03 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec mapd func listA result=
                match listA with
                |[] -> List.rev result
                |head::tail -> mapd func tail ((func head)::result)
            mapd f xs []// write a map which applies f to each element
