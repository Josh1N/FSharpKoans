...
module ``10: The Good Kind of Discrimination`` = 
    ...
    let ``04 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = 
            let fourth =Node ("4", Empty, Empty)
            let third =Node ("4", fourth, fourth)
            let second =Node ("4", third, third)
            let first =Node ("1", second, second)
            first// <-- you may want to spread this over multiple lines and/or let-bindings ...!
